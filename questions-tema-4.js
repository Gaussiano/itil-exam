// Tema 4 — Los 7 Principios Rectores de ITIL
window.QUESTIONS_T4 = [
  {
    topic: 4,
    q: "¿Qué es un principio rector según ITIL?",
    options: [
      "Una norma obligatoria que debe cumplirse exactamente",
      "Una recomendación que guía a una organización en cualquier circunstancia, aunque aparezcan cambios o nuevos retos",
      "Un procedimiento operativo estándar",
      "Una métrica de rendimiento clave"
    ],
    correct: 1,
    explanation: "Principio rector: recomendación que guía a una organización en cualquier circunstancia, aunque aparezcan cambios o nuevos retos. Se acepta como principio universal y duradero.",
    ref: "Tema 4, slide 0"
  },
  {
    topic: 4,
    q: "¿Cuántos principios rectores define ITIL 4?",
    options: [
      "5",
      "6",
      "7",
      "9"
    ],
    correct: 2,
    explanation: "Son 7 principios rectores: Enfoque en el Valor, Empieza donde estás, Progresa iterativamente con feedback, Colabora y promueve la visibilidad, Piensa y trabaja de forma holística, Mantener la simplicidad y practicidad, y Optimiza y automatiza.",
    ref: "Tema 4, slide 1"
  },
  {
    topic: 4,
    q: "¿Cuál de los siguientes NO es un principio rector de ITIL?",
    options: [
      "Enfoque en el valor",
      "Progresa iterativamente con feedback",
      "Optimiza y automatiza",
      "Maximiza la documentación"
    ],
    correct: 3,
    explanation: "Maximizar la documentación no es un principio rector. Los 7 principios son: Foco en el valor, Empieza donde estás, Iterativo con feedback, Colabora y promueve visibilidad, Holístico, Simplicidad y practicidad, y Optimiza/automatiza.",
    ref: "Tema 4, slide 1"
  },
  {
    topic: 4,
    q: "¿Qué principio rector implica que todo lo que hacemos se dirige a entregar valor a los interesados?",
    options: [
      "Enfoque en el Valor",
      "Piensa y trabaja de forma holística",
      "Colabora y promueve la visibilidad",
      "Optimiza y automatiza"
    ],
    correct: 0,
    explanation: "Enfoque en el Valor: todo lo que hacemos se dirige, directa o indirectamente, a entregar valor a los interesados. Nos enfocamos en la experiencia de usuarios y clientes.",
    ref: "Tema 4, slide 2"
  },
  {
    topic: 4,
    q: "Una empresa va a digitalizar un proceso. Existen procedimientos manuales que funcionan. ¿Qué principio rector aplica para no descartarlos sin más?",
    options: [
      "Empieza donde estás",
      "Optimiza y automatiza",
      "Mantén la simplicidad",
      "Piensa holísticamente"
    ],
    correct: 0,
    explanation: "Empieza donde estás: no empezamos de cero si podemos aprovechar trabajo ya realizado. Los proyectos arrancan sobre líneas base que comprendemos.",
    ref: "Tema 4, slide 3"
  },
  {
    topic: 4,
    q: "Una organización quiere mejorar un servicio en pasos pequeños y obtener feedback. ¿Qué principio aplica?",
    options: [
      "Progresa iterativamente con feedback",
      "Empieza donde estás",
      "Optimiza y automatiza",
      "Colabora y promueve la visibilidad"
    ],
    correct: 0,
    explanation: "Progresa iterativamente con feedback: sin \"big bangs\" ni planificaciones predictivas a largo plazo. Trabajar en base a sprints y feedback.",
    ref: "Tema 4, slide 4"
  },
  {
    topic: 4,
    q: "¿Qué quiere decir \"radiadores de información en lugar de neveras\"?",
    options: [
      "Que el material debe estar refrigerado",
      "Que la información debe difundirse y ser visible, no guardarse oculta",
      "Que sólo los managers tienen acceso al estado del proyecto",
      "Que la documentación debe destruirse cuando deja de ser útil"
    ],
    correct: 1,
    explanation: "Pertenece a \"Colabora y promueve la visibilidad\". Radiadores de información (visibles para todos) en lugar de \"neveras\" (información escondida).",
    ref: "Tema 4, slide 5"
  },
  {
    topic: 4,
    q: "\"El conjunto total de elementos de un sistema es mucho más que la suma de cada uno individualmente.\" ¿A qué principio corresponde?",
    options: [
      "Enfoque en el valor",
      "Mantener la simplicidad",
      "Piensa y trabaja de forma holística",
      "Empieza donde estás"
    ],
    correct: 2,
    explanation: "Piensa y trabaja de forma holística: el conjunto total de elementos de un sistema es mucho más que la suma de cada uno individualmente. Todo interactúa con todo.",
    ref: "Tema 4, slide 6"
  },
  {
    topic: 4,
    q: "\"Si algo no aporta valor, elimínalo.\" ¿A qué principio corresponde?",
    options: [
      "Mantener la simplicidad y practicidad",
      "Empieza donde estás",
      "Progresa iterativamente",
      "Piensa holísticamente"
    ],
    correct: 0,
    explanation: "Mantener la simplicidad y practicidad: si no aporta, no lo hagas. Diseño tan sencillo como sea posible, sin intentar cubrir el 100% de los casos excepcionales.",
    ref: "Tema 4, slide 39"
  },
  {
    topic: 4,
    q: "\"No Gold Plating\" (no añadir funcionalidades de más) es un consejo del principio…",
    options: [
      "Optimiza y automatiza",
      "Mantener la simplicidad y practicidad",
      "Enfoque en el valor",
      "Empieza donde estás"
    ],
    correct: 1,
    explanation: "Pertenece a \"Mantener la simplicidad y practicidad\": no gold plating, solución para requisitos actuales, no \"por si acaso\".",
    ref: "Tema 4, slide 7"
  },
  {
    topic: 4,
    q: "¿Qué hay que hacer ANTES de automatizar, según el principio Optimiza y automatiza?",
    options: [
      "Comprar la mejor tecnología disponible",
      "Simplificar y optimizar",
      "Contratar más personal",
      "Documentar exhaustivamente"
    ],
    correct: 1,
    explanation: "Consejo del principio: Simplifica y optimiza ANTES de automatizar. Automatizar un proceso ineficiente solo perpetúa la ineficiencia.",
    ref: "Tema 4, slide 49"
  },
  {
    topic: 4,
    q: "Una métrica nunca debería convertirse en…",
    options: [
      "Información de apoyo",
      "Un objetivo en sí mismo",
      "Pública dentro de la organización",
      "Comparable con otras"
    ],
    correct: 1,
    explanation: "Las métricas se toman para saber algo. Nunca una métrica puede convertirse en un objetivo.",
    ref: "Tema 4, slide 20"
  },
  {
    topic: 4,
    q: "Pregunta clave del principio \"Enfoque en el Valor\":",
    options: [
      "\"¿Cuánto cuesta?\"",
      "\"¿Para quién lo hacemos?\"",
      "\"¿Quién es el responsable?\"",
      "\"¿Cuándo se entrega?\""
    ],
    correct: 1,
    explanation: "Todas las actividades llevan, directa o indirectamente, a crear valor para la organización, sus clientes u otros interesados. Pregunta clave: \"¿para quién lo hacemos?\".",
    ref: "Tema 4, slide 8"
  },
  {
    topic: 4,
    q: "¿Qué es el Producto Mínimo Viable (MVP) en el contexto del principio \"Progresa iterativamente\"?",
    options: [
      "El producto con funcionalidades premium",
      "Una versión del producto final que permite aprender y validar el avance con el mínimo esfuerzo",
      "El presupuesto mínimo necesario",
      "Un prototipo de papel para presentaciones"
    ],
    correct: 1,
    explanation: "En cada iteración se trata de obtener un Producto Mínimo Viable: una versión del producto final que permite aprender y validar el avance con el mínimo esfuerzo.",
    ref: "Tema 4, slide 29"
  },
  {
    topic: 4,
    q: "\"Colaboración no significa…\"",
    options: [
      "comunicación",
      "consenso",
      "transparencia",
      "trabajo conjunto"
    ],
    correct: 1,
    explanation: "Consejo del principio Colabora y promueve la visibilidad: colaboración no significa consenso. Hay que adaptar el mensaje y el medio en función del oyente y tomar decisiones basadas en datos visibles.",
    ref: "Tema 4, slide 36"
  },
  {
    topic: 4,
    q: "Según ITIL, ¿hay que automatizar todo lo posible?",
    options: [
      "Sí, todo proceso debe automatizarse",
      "No: automatizar aquello en lo que los humanos no aporten un valor añadido; no todo se puede automatizar",
      "Sólo los procesos financieros deben automatizarse",
      "Únicamente la atención al cliente"
    ],
    correct: 1,
    explanation: "Optimiza y automatiza: automatiza aquello en lo que los humanos no aporten un valor añadido. No todo se puede automatizar. Las personas tendrán más tiempo para aportar creatividad e imaginación.",
    ref: "Tema 4, slide 44"
  },
  {
    topic: 4,
    q: "¿Cuál de las siguientes es una recomendación del principio \"Empieza donde estás\"?",
    options: [
      "Inventar la rueda cada vez",
      "Evaluar lo existente de forma objetiva",
      "Borrar todas las métricas anteriores",
      "Empezar siempre con tecnología nueva"
    ],
    correct: 1,
    explanation: "Consejos para \"Empieza donde estás\": evalúa lo existente de forma objetiva, no inventes la rueda cada vez, aplica gestión de riesgos, a veces no se puede reutilizar.",
    ref: "Tema 4, slide 22"
  },
  {
    topic: 4,
    q: "Según ITIL, en un timebox, hacer las cosas \"rápido\" significa…",
    options: [
      "De baja calidad",
      "Precipitado",
      "No significa ni precipitado ni de baja calidad",
      "Sin documentación"
    ],
    correct: 2,
    explanation: "Rápido en un timebox no significa \"precipitado\" o \"de baja calidad\".",
    ref: "Tema 4, slide 29"
  },
  {
    topic: 4,
    q: "\"Hacer menos cosas, pero hacerlas mejor\" es un consejo de…",
    options: [
      "Optimiza y automatiza",
      "Mantener la simplicidad y practicidad",
      "Progresa iterativamente",
      "Piensa holísticamente"
    ],
    correct: 1,
    explanation: "Consejos para \"Mantener la simplicidad y practicidad\": el arte de la simplicidad, hacer menos cosas pero hacerlas mejor, respetar el tiempo de los implicados.",
    ref: "Tema 4, slide 43"
  },
  {
    topic: 4,
    q: "Si tenemos pocas incidencias en un servicio, según el principio \"Empieza donde estás\":",
    options: [
      "Es indicador inequívoco de que el servicio funciona bien",
      "Es indicador inequívoco de que el servicio es deficiente",
      "No es indicador conclusivo por sí solo: hay que interpretar las métricas",
      "Hay que dejar de medir las incidencias"
    ],
    correct: 2,
    explanation: "Una métrica aislada no da la respuesta. Pocas incidencias podrían significar que va bien… o que los usuarios no las reportan. Hay que analizar las métricas en contexto.",
    ref: "Tema 4, slide 19"
  },
  {
    topic: 4,
    q: "Para aplicar el principio \"Enfoque en el valor\", uno de los consejos es:",
    options: [
      "Centrar todo en la rentabilidad financiera",
      "Conocer cómo los consumidores del servicio usan cada servicio",
      "Eliminar a los empleados que no aportan valor",
      "Tratar a todos los clientes por igual sin segmentar"
    ],
    correct: 1,
    explanation: "Consejos: conocer cómo los consumidores usan cada servicio, promover que los empleados piensen en la aportación de valor, enfocarse en el valor en la construcción, operación y mejora.",
    ref: "Tema 4, slide 12"
  },
  {
    topic: 4,
    q: "¿Cuál es el medio de comunicación MÁS rico para colaborar con un interesado clave?",
    options: [
      "E-mail",
      "Documentos",
      "Conversación telefónica",
      "Cara a cara + radiador de información"
    ],
    correct: 3,
    explanation: "Los medios de comunicación, de menos a más ricos: documentos, e-mail, conversación telefónica, videoconferencia, cara a cara, y cara a cara + radiador de información.",
    ref: "Tema 4, slide 33"
  },
  {
    topic: 4,
    q: "Una de las ventajas de aplicar iteraciones cortas + feedback es:",
    options: [
      "Eliminar la necesidad de planificar",
      "Descubrir y responder a errores antes, y permitir respuestas más rápidas",
      "Aumentar la duración del proyecto",
      "Reducir la implicación del cliente"
    ],
    correct: 1,
    explanation: "Iterar y obtener feedback permite: flexibilidad (la siguiente iteración es adaptable), respuestas más rápidas, descubrir y responder a errores antes, mejora continua.",
    ref: "Tema 4, slide 27"
  },
  {
    topic: 4,
    q: "El término \"parálisis por análisis\" se asocia con el principio…",
    options: [
      "Progresa iterativamente con feedback",
      "Empieza donde estás",
      "Optimiza y automatiza",
      "Piensa holísticamente"
    ],
    correct: 0,
    explanation: "Consejo del principio iterativo: evita la \"parálisis por análisis\". Al inicio obtener la información suficiente para empezar, no toda la necesaria para terminar.",
    ref: "Tema 4, slide 28"
  },
  {
    topic: 4,
    q: "Para automatizar correctamente, el primer paso es:",
    options: [
      "Comprar las mejores herramientas",
      "Entender el contexto: ¿por qué queremos automatizar?",
      "Eliminar el proceso actual",
      "Despedir a personal manual"
    ],
    correct: 1,
    explanation: "Pasos del camino hacia la automatización: 1) entender el contexto; 2) analizar la línea base actual; 3) acordar el objetivo; 4) asegurar la participación de los interesados; 5) automatizar de forma iterativa; 6) monitorizar el impacto.",
    ref: "Tema 4, slide 45"
  },
  {
    topic: 4,
    q: "Identificar cuellos de botella y mostrar cómo fluye el trabajo se asocia con…",
    options: [
      "Optimiza y automatiza",
      "Empieza donde estás",
      "Colabora y promueve la visibilidad",
      "Piensa holísticamente"
    ],
    correct: 2,
    explanation: "\"Dar visibilidad al trabajo\" pertenece a Colabora y promueve la visibilidad: irradiar el avance del trabajo, mostrar cómo fluye, identificar cuellos de botella (o capacidad excesiva) y descubrir desperdicios.",
    ref: "Tema 4, slide 35"
  },
  {
    topic: 4,
    q: "¿Cuál es la definición correcta de \"interesado\" según se trata en el principio de Colaboración?",
    options: [
      "Aquél que financia el proyecto",
      "Aquél que se ve impactado por el proyecto",
      "El equipo de desarrollo",
      "El proveedor externo"
    ],
    correct: 1,
    explanation: "Interesado: aquél que se ve impactado por el proyecto. \"Impactado\" es mejor traducción que \"interesado\".",
    ref: "Tema 4, slide 32"
  },
  {
    topic: 4,
    q: "El consejo \"Reconoce la complejidad de algunos sistemas\" y \"Aplicar Design Thinking\" se asocia a:",
    options: [
      "Piensa y trabaja de forma holística",
      "Empieza donde estás",
      "Optimiza y automatiza",
      "Enfoque en el valor"
    ],
    correct: 0,
    explanation: "Consejos para Piensa y trabaja de forma holística: reconoce la complejidad de algunos sistemas, la colaboración es clave, investigar la interacción entre elementos, aplicar Design Thinking.",
    ref: "Tema 4, slide 38"
  },
  {
    topic: 4,
    q: "Cuando ITIL dice \"Empieza donde estás\", quiere decir…",
    options: [
      "Quédate en el sitio físico actual",
      "Construir algo nuevo no implica desechar todo lo existente: reutilizar, mejorar, potenciar lo que ya tienes",
      "No emprender nuevos proyectos",
      "Esperar a tener todos los recursos"
    ],
    correct: 1,
    explanation: "Empieza donde estás: construir algo nuevo no implica desechar todo lo existente. ¿Podemos reutilizarlo? ¿Mejorarlo? ¿Potenciarlo?",
    ref: "Tema 4, slide 17"
  },
  {
    topic: 4,
    q: "El término \"omnicanalidad\" en el contexto de la experiencia de cliente significa…",
    options: [
      "Tener muchos canales separados",
      "Una experiencia coherente y consistente del cliente independientemente del canal",
      "Que el cliente puede usar solo un canal",
      "Vender por todos los canales posibles"
    ],
    correct: 1,
    explanation: "La experiencia de cliente (CX) y omnicanalidad implican que el cliente \"se siente\" atendido igual por cualquier medio. El cliente interactúa con la empresa de forma consistente.",
    ref: "Tema 4, slide 10"
  }
];
